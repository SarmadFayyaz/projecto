...
{{--                    <textarea rows="1" class="form-control task_note_add_ajax mb-3" data-id="" data-task-id="{{ $task->id }}" placehoder="{{ __('header.add_note') }}"></textarea>--}}
                    @forelse($task->taskNote as $note)
                        {{--                        <p class="w-100 mb-2 pt-2 pb-2 pl-3 pr-3 rounded" style="line-height:16px; display: flex;">--}}
                        {{--                            {{ $note->notes }}--}}
                        {{--                        </p>--}}
                        <textarea rows="1" class="form-control task_note_add_ajax {{ ($loop->last && !$loop->first) ? '' : 'mb-3' }}" data-task-id="{{ $task->id }}" data-id="{{ $note->id }}">{{ $note->notes }}</textarea>
                    @empty
                        <textarea rows="1" class="form-control task_note_add_ajax mb-3" data-task-id="{{ $task->id }}"></textarea>
                    @endforelse
                </div>
            </div>
        </div>
        @if($task->project->project_leader == Auth::user()->id)
            <div class="col-12 text-right">
                ...
                @elseif($task->progress == 100 && ($task->status == 'approved' || $task->status == 'pending_completion'))
