            ...
            <div class="row justify-content-center">
                <div class="col-md-10">
                    <div class="card ">
                        ...
                        <form method="post" action="{{ route('company.user.update', $user->id) }}" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            <div class="card-body ">
                                @yield('message')
                                        ...
                                        <div class="form-group @error('role') has-danger @enderror">
                                            <select class="selectpicker" name="role" id="role" data-style="select-with-transition" data-size="4" data-width="100%" title="{{ __('header.choose_role') }}">
                                                ...
                                                @foreach($roles as $role)
                                                    <option value="{{ $role->id }}" {{ ($role->id == old('role')) ? 'selected' : ($user->roles->contains($role->id) ? 'selected' : '') }}>
                                                        {{ $role->name }}
                                                    </option>
                                                @endforeach
                                            </select>
